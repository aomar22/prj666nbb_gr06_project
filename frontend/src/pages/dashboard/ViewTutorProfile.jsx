...
export default function ViewTutorProfile() {
		...
		if (user?.role === "LEARNER") {
			getMySessions()
				.then((sessions) => {
					...
					setCanReview(tutorSessions.length > 0 && !alreadyReviewed);
				})
				.catch(() => {});
		}
	}, [tutor?.id]);
		...
		return (
			<div style={styles.container}>
				...
				<main style={styles.mainContent}>
					<p style={styles.noTutor}>
						No tutor selected.{" "}
						<button
							type='button'
							style={styles.backLink}
							onClick={() => navigate("/dashboard/learner/find-tutors/results")}
						>
							Go back to results
						</button>
					</p>
	...
	const teachingMode =
		Array.isArray(tutor.teachingMode) && tutor.teachingMode.length
			? tutor.teachingMode.map((m) => TEACHING_MODE_LABELS[m] || m).join(" / ")
			: "Online / In-person";
	const campus =
		Array.isArray(tutor.campus) && tutor.campus.length
			? tutor.campus.join(", ")
			: tutor.campus || "Newnham";
	...
	const courses =
		Array.isArray(tutor.coursesOffered) && tutor.coursesOffered.length
			...
			: tutor.program || "—";
	const programLabel = Array.isArray(tutor.program)
		? tutor.program.join(", ")
		: tutor.program || "—";
	const sessionType = tutor.sessionType
		? SESSION_TYPE_LABELS[tutor.sessionType] || tutor.sessionType
		: "One-on-One";
	const aboutText =
		tutor.bio ||
			...
			<main style={styles.mainContent}>
				...
				<div style={styles.profileCard}>
									...
									<button
										...
										onClick={() =>
											navigate("/dashboard/learner/booking", {
												state: { tutorId: tutor.id ?? tutor.userId, tutor },
											})
										}
									>
										Book session
					...
					<section style={styles.section}>
						...
						<p style={styles.detailRow}>
							<strong>Teaching Mode:</strong> {teachingMode}
						</p>
						<p style={styles.detailRow}>
            ...
            '
                  ...
                  '
								>
									<span
										className={
											star <= activeRating ? "text-[#E0B100]" : "text-[#B9B9B9]"
										}
									>
										★
									</span>
